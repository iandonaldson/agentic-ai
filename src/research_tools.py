...
session.headers.update({"User-Agent": "LF-ADP-Agent/1.0 (mailto:your.email@example.com)"})
...
def pdf_bytes_to_text(pdf_bytes: bytes, max_pages: Optional[int] = None) -> str:
    # 1) PyMuPDF
    try:
        # Avoid a static import of `fitz` so mypy doesn't complain when library
        # stubs/py.typed are not available. Use a TYPE_CHECKING import for
        # dev-time typing and dynamic import at runtime.
        if TYPE_CHECKING:  # pragma: no cover - only for type checkers
            import fitz  # type: ignore

            fitz_mod: Any = fitz  # type: ignore[name-defined]
        else:
            fitz_mod = importlib.import_module("fitz")

        out = []
        with fitz_mod.open(stream=pdf_bytes, filetype="pdf") as doc:
            n = len(doc)
